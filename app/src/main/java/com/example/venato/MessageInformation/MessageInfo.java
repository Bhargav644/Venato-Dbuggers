...
public class MessageInfo extends AppCompatActivity {
    ...
    public MessageInfo(Context _context){
        ...
        DatabaseReference reference= FirebaseDatabase.getInstance().getReference().child("Tweaks").child("WindowSize");
    ...
    public List<Double> calScore(){
        ...
        Log.i("Check",windowSize+" ");
        List<Sms> allMessages=alignAllSms(windowSize);
    ...
    public List<Sms> alignAllSms(long windowSize){
        ...
        if(c.moveToFirst()){
            for(int i=0;i<totalSms;i++){
                ...
                if(!Objects.equals(curr_Folder, "None") && getDaysDifference(dateOfCall,currDate)<=windowSize){
                    objSms=new Sms();
    ...
    public Double findAmountFromMessage(String msg){
        ...
        int index2=-1;
        int index1=msg.indexOf("Rs");
        if(index1==-1){
            index2=msg.indexOf("INR");
            if(index2==-1) {
                return 0.0;
            }
        }
