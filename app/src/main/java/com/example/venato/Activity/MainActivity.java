...
import com.example.venato.Database.DBClass;
import com.example.venato.Database.DynamicVariables;

import com.example.venato.MessageInformation.MessageInfo;
import com.example.venato.R;
import com.example.venato.SystemInformation.SystemInfo;
...
//        setContentView(R.layout.activity_main);
        ...
        //System Info
        SystemInfo regSystem=new SystemInfo(this);
        String systemId=regSystem.getSystemId();
        ...
        DynamicVariables var=new DynamicVariables(this);

        double CREDIT_SCORE_WITHOUT_PERCENTAGE=behaviouralScore*var.weights[0]+financialScore*var.weights[1];//maximum can be 62.4;
        Log.i("CHeck", CREDIT_SCORE_WITHOUT_PERCENTAGE+" ");
//        double CREDIT_SCORE_WITH_PERCENTAGE = (CREDIT_SCORE_WITHOUT_PERCENTAGE*100)/62.4;//maximum can be 62.4
        double CREDIT_SCORE_WITH_PERCENTAGE = behaviouralScorePercent*0.1+financialScorePercent*0.9;//maximum can be 62.4
        ...
        try{
            ...
            db.setReadUnreadRatio(readUnreadRatio);
            db.setBehaviouralScore(behaviouralScore);
            db.setBatteryScore(newBatteryScore);
            db.setAppScore(appScore);
            db.setCallScore(callParameters.get(0));
            db.setCreditScoreWithPercentage(CREDIT_SCORE_WITH_PERCENTAGE);
            db.setCreditScoreWithoutPercentage(CREDIT_SCORE_WITHOUT_PERCENTAGE);
            db.setClearanceScore(clearanceScore);
            db.setFinancialScore(financialScore);
            db.setDueScore(messageParameter.get(0));
            db.setLastBattery(batteryPct);
        }
        catch(Exception e){
        ...
        String newId=db.getSystemId().replace('.','_');
        userInfoRef.child(newId).setValue(db);
